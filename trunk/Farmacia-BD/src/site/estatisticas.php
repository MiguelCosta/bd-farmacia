...
<table width="776" height="592" border="0" cellpadding="0" cellspacing="0" class="frame" id="Table_01">
	...
	<tr>
	  <td height="391" colspan="12"><table width="770" height="243" border="0" align="center">
	    <tr>
	      <td height="46" align="center" class="tableEstGray"><h3> PRODUTOS MAIS VENDIDOS</h3></td>
	      <td align="center" class="tableEstWhite"><h3>PRODUTOS MENOS VENDIDOS</h3></td>
	      <td align="center"class="tableEstGray"><h3>VENDAS VALOR MAIS ALTO</h3></td>
	      <td align="center" class="tableEstWhite"><h3>VENDAS VALOR MAIS BAIXO</h3></td>
	      </tr>
	    <tr>
	      <td valign="top" class="tableEstGray">
		  <?php
          $connect = ocilogon("MIGUEL","433433","localhost");
		  
		  $sql = "SELECT * FROM (SELECT produto, count(*) as refs FROM (SELECT PRODUTO FROM VENDAS) GROUP BY PRODUTO ORDER BY refs DESC) WHERE ROWNUM <=10";
		  
			$stmt = ociparse($connect,$sql);
			ociexecute($stmt,OCI_DEFAULT);
			
			
			echo "<table width='188' class='tableEstGray'>";
				echo "<tr>";
					echo "<td>";
					echo "Produto";
					echo "</td>";
	      ...
	      <td height="191" valign="top" class="tableEstWhite">
          
          <?php
          $connect = ocilogon("MIGUEL","433433","localhost");
		  
		  $sql = "SELECT * FROM (SELECT produto, count(*) as refs FROM (SELECT PRODUTO FROM VENDAS) GROUP BY PRODUTO ORDER BY refs ASC) WHERE ROWNUM <=10";
		  
			$stmt = ociparse($connect,$sql);
			ociexecute($stmt,OCI_DEFAULT);
			
			
			echo "<table width='188' class='tableEstWhite'>";
				echo "<tr>";
					echo "<td>";
					echo "Produto";
					echo "</td>";
	      ...
	      <td valign="top" class="tableEstGray"><span class="tableEstWhite">
	      <?php
          $connect = ocilogon("MIGUEL","433433","localhost");
		  
		  $sql = "SELECT * FROM (SELECT NUMERO, SUM(MONTANTE_TOTAL)as montante FROM LISTA_VENDAS GROUP BY NUMERO ORDER BY montante DESC) WHERE ROWNUM <=10";
	      ...
	      <td valign="top" class="tableEstWhite"><span class="tableEstGray">
	        <?php
          $connect = ocilogon("MIGUEL","433433","localhost");
		  
		  $sql = "SELECT * FROM (SELECT NUMERO, SUM(MONTANTE_TOTAL)as montante FROM LISTA_VENDAS GROUP BY NUMERO ORDER BY montante ASC) WHERE ROWNUM <=10";
